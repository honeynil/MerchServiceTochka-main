...
import (
	...
	func TestMerchService_Login(t *testing.T) {
		...
		kafkaProducerUsers := kafkamocks.NewMockKafkaProducer(ctrl)
		kafkaProducerTransactions := kafkamocks.NewMockKafkaProducer(ctrl)

		ctx := context.Background()
		jwtSecret := "secret"
		service := NewMerchService(userRepo, merchRepo, transactionRepo, redisClient, kafkaProducerUsers, kafkaProducerTransactions, jwtSecret)
